...
public class AxiomsAdding {
    ...
    public static void addingSeveralFromBd(OWLOntology ont, OWLOntologyManager manager, OWLDataFactory df, String ns, OWLIndividual ind1, List<String> ind2Name, OWLObjectProperty property){
        for (String i: ind2Name)  {
            OWLAxiom Ax = df.getOWLObjectPropertyAssertionAxiom(property, ind1, df.getOWLNamedIndividual(IRI.create(ns+i)));
            AddAxiom Ad = new AddAxiom(ont, Ax);
            manager.applyChange(Ad);
        }
    }
